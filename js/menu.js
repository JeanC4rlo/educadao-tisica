...
function montarMenu() {
    ...
    if (USUARIO.tipoUsuario == "Organizador") {
        TELA_CRIACAO.innerHTML = `<button onclick="window.location = 'criar-torneio.html'">Criar novo torneio</button>`;
    }
}

function torneioTarget(torneio) {
    localStorage.setItem("torneio-alvo", JSON.stringify(torneio));
}
...
function editarTorneio(id) {
    const TORNEIO = getTorneioById(id);
    torneioTarget(TORNEIO);
    window.location = "editar-torneio.html";
}
